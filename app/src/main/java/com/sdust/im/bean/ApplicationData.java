...
public class ApplicationData {
	...
	public void setChatMessagesMap(Map<Integer, List<ChatEntity>> mChatMessagesMap) {
	...
	public void friendRequestArrived(TranObject mReceivedRequest) {
		...
		if (mReceivedRequest.getResult() == Result.MAKE_FRIEND_REQUEST) {
			...
			messageEntity.setMessageType(MessageTabEntity.MAKE_FRIEND_RESPONSE_ACCEPT);
			...
			messageEntity.setMessageType(MessageTabEntity.MAKE_FRIEND_RESPONSE_REJECT);
	...
	public void messageArrived(TranObject tran) {
		...
		if (chatList == null) {
			chatList = ImDB.getInstance(mContext).getChatMessage(chat.getSenderId());
